...
local objects = {
    ...
    ["Instance61"] = Instance.new("Frame"); -- Neon
    ["Instance62"] = Instance.new("TextLabel"); -- Topbar
    ["Instance63"] = Instance.new("UIAspectRatioConstraint"); -- UIAspectRatioConstraint
    ["Instance64"] = Instance.new("LocalScript"); -- NewTest
    ["Instance65"] = Instance.new("ModuleScript"); -- UI
    ["Instance66"] = Instance.new("ModuleScript"); -- TestingAPI
    ["Instance67"] = Instance.new("TextLabel"); -- Caption
    ["Instance68"] = Instance.new("UIPadding"); -- UIPadding
};
...
do -- Set properties
    ...
    objects["Instance61"]["Name"] = "Neon";
    objects["Instance61"]["Size"] = UDim2.new(1, 0, 0, 5);
    objects["Instance61"]["Parent"] = objects["Instance1"];
    objects["Instance61"]["ZIndex"] = 2;
    objects["Instance61"]["BorderSizePixel"] = 0;
    objects["Instance61"]["BackgroundColor3"] = Color3.new(0.392157, 0.392157, 0.392157);

    objects["Instance62"]["FontSize"] = Enum.FontSize.Size14;
    objects["Instance62"]["Parent"] = objects["Instance1"];
    objects["Instance62"]["AnchorPoint"] = Vector2.new(0.5, 0);
    objects["Instance62"]["ZIndex"] = 3;
    objects["Instance62"]["BorderSizePixel"] = 0;
    objects["Instance62"]["Size"] = UDim2.new(0.949999988, 0, 0.075000003, 0);
    ...
    objects["Instance62"]["Text"] = "Cherry's Environment Test <font color=\"rgb(255,150,150)\">[ V2 ]</font>";
    objects["Instance62"]["Position"] = UDim2.new(0.5, 0, 0.0130000003, 5);
    ...
    objects["Instance62"]["Name"] = "Topbar";
    objects["Instance62"]["TextXAlignment"] = Enum.TextXAlignment.Left;
    objects["Instance62"]["TextSize"] = 14;
    objects["Instance62"]["FontFace"] = Font.new("rbxasset://fonts/families/TitilliumWeb.json", Enum.FontWeight.Regular, Enum.FontStyle.Normal);
    objects["Instance62"]["TextScaled"] = true;
    objects["Instance62"]["TextWrap"] = true;

    objects["Instance63"]["Parent"] = objects["Instance1"];
    objects["Instance63"]["AspectRatio"] = 1.899999976158142;

    objects["Instance64"]["Parent"] = objects["Instance1"];
    objects["Instance64"]["Name"] = "NewTest";

    objects["Instance65"]["Parent"] = objects["Instance64"];
    objects["Instance65"]["Name"] = "UI";

    objects["Instance66"]["Parent"] = objects["Instance64"];
    objects["Instance66"]["Name"] = "TestingAPI";

    objects["Instance67"]["FontFace"] = Font.new("rbxasset://fonts/families/TitilliumWeb.json", Enum.FontWeight.Regular, Enum.FontStyle.Normal);
    objects["Instance67"]["TextColor3"] = Color3.new(1, 1, 1);
    objects["Instance67"]["Parent"] = objects["Instance0"];
    objects["Instance67"]["Text"] = "TextLabel";
    objects["Instance67"]["BackgroundColor3"] = Color3.new(0.0588235, 0.0588235, 0.0588235);
    objects["Instance67"]["AnchorPoint"] = Vector2.new(1, 1);
    objects["Instance67"]["BorderColor3"] = Color3.new(0.392157, 0.392157, 0.392157);
    objects["Instance67"]["Name"] = "Caption";
    objects["Instance67"]["Size"] = UDim2.new(0, 10, 0, 4);
    objects["Instance67"]["TextWrapped"] = true;
    objects["Instance67"]["ZIndex"] = 999999999;
    objects["Instance67"]["TextScaled"] = true;
    objects["Instance67"]["TextWrap"] = true;

    objects["Instance68"]["PaddingTop"] = UDim.new(0.200000003, 0);
    objects["Instance68"]["PaddingBottom"] = UDim.new(0.200000003, 0);
    objects["Instance68"]["PaddingRight"] = UDim.new(0.100000001, 0);
    objects["Instance68"]["PaddingLeft"] = UDim.new(0.100000001, 0);
...
local modules do
    ...
    modules[objects["Instance66"]] = function()
        local script = objects["Instance66"];
...
return {
    ...
    [{Names = {"hookfunction", "replaceclosure"}, HasSkid = true, Important = true, Group = "Hooks"}] = function(fc)
        ...
        if success ~= 3 then -- yeah, this is already bad function, just killing executor even more
            ...
            local copy = fc(genv.ElapsedTime, function()
                return 1488
            end)
    ...
    [{Names = {"getgenv"}, HasSkid = true, Important = true, Group = "Environments"}] = function(fc)
        ...
        if fc().CETGLOBAL3 ~= rng then
            return false, "Failed to share value to current global environment"
        end
    end,
    ...
    [{Names = {"getinstances"}, HasSkid = true, Important = true, Group = "Instances"}] = function(fc)
        ...
        if #got <= #game:GetDescendants() then
            return false, "Didn't get all instances", 1
        end
    ...
    [{Names = {"hookmetamethod"}, HasSkid = true, Important = true, Group = "Hooks"}] = function(fc)
        ...
        if cc then
            local success = false
            local old; old = fc(game, "__namecall", function(...)
                if not cc() then
                    success = true
                end
                return old(...)
    ...
    modules[objects["Instance65"]] = function()
...
local function cetLoadFail()
    game:GetService("StarterGui"):SetCore("SendNotification", {Title = "CET error", Text = "CET Unable to load"})
    game:GetService("StarterGui"):SetCore("SendNotification", {Title = "CET error", Text = "Your executor has messed instances / capabilities"})
...
local function doTest()
    ...
    if not pcall(ui.CastLog, ui, "Info", "Running the test") then
        return cetLoadFail()
    end
...
ui:ConsoleLog("[!] UI fully loaded; Running first test")
ui:ConsoleLog("")

if not pcall(doTest) then
    return cetLoadFail()
end
